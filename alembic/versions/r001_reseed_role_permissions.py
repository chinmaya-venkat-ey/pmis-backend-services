...
Revision ID: r001_reseed_role_permissions
Revises: u1a000000002
Create Date: 2026-05-24

Why:
  The VM DB may have been created by the monolith which uses different
  permission codes, OR the seed in u1a000000002 ran before these roles
  existed. This migration uses ON CONFLICT DO NOTHING to be idempotent
  — safe to run multiple times.

  Bug #13: org_admin and project_admin could not view User/Org Management
  pages because the required permission codes were missing from the
  role_permissions table in the deployed DB.
"""
...
revision: str = "r001_reseed_role_permissions"
...
# Collect every code we reference so they exist before FK insert.
...
def upgrade() -> None:
    ...
    # 1. Ensure every permission code row exists (idempotent).
    ...
    # 3. Insert missing grants (idempotent — ON CONFLICT DO NOTHING).
...
def downgrade() -> None:
    # Intentional no-op: removing permissions would lock out live users.
    pass
