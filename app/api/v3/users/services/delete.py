...
(via PATCH status='active' or POST /restore) restores the user with
their full mapping history.

Two admin-protection guards are enforced here so the API surface can
never lock the system out of itself:

  - **Self-delete is forbidden.** A logged-in admin cannot delete their
    own account (would also revoke their session). Returns
    ``authorization_error`` (403).
  - **Last active admin is protected.** Deleting an admin row is
    refused if no OTHER active admin would remain. Returns
    ``validation_error`` (422). Recovery from a fully-locked-out state
