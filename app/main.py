"""pmis-contract-management — FastAPI application entry point.

Mounts:
  - Business routers under ``/contract/*``
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="pmis-contract-management",
        version="0.1.0",
        description=(
            "Contract onboarding, SLA definition, DSL generation, ASL parsing, "
            "severity evaluation, LD calculation, and reporting. "
            "Owns the `contract` schema. Reads cross-schema mirrors of "
            "`users.*` (auth + RBAC) and `masters.*`."
        ),
