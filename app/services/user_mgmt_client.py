...
(that would bypass user-management's own checks). A 401 from user-management
means the token is anonymous / expired / revoked; any other failure
(unreachable, 5xx) raises so the middleware can fail closed.
"""
...
class UserMgmtClient:
    ...
    Construct per request; the ``httpx.Client`` is created inside the call
    (no persistent pool — matches the other PMIS service clients).
    """
    ...
    def fetch_authz_context(self, authorization: str) -> Optional[dict]:
        ...
        if not self.base_url:
            # Standalone/dev mode: no PDP configured → treat as anonymous.
            return None
