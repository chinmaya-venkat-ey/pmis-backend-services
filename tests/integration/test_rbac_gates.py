...
def test_login_reachable_anonymous(anonymous_client, app, fake_db_session):
    ...
    try:
        ...
        if resp.status_code == 401:
            assert resp.json()["code"] in {"INVALID_CREDENTIALS", "AUTH_REQUIRED"}
    finally:
...
def test_reader_cannot_delete_user(reader_client):
    resp = reader_client.delete("/user/users/some-uuid/delete")
    assert resp.status_code == 403
...
def test_admin_can_list_users(client, app, fake_db_session):
    ...
    # repository.list_ returns (rows, total). Mock at the controller boundary
    # by overriding the controller dep instead — it's cleaner here.
    ...
    try:
        resp = client.get("/user/users/list")
        assert resp.status_code == 200
