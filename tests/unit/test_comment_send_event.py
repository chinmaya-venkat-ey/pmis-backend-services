...
def test_service_rejects_too_large_attachment(monkeypatch):
    ...
    atts = [{"url": "https://x/y.pdf", "filename": "y.pdf", "sizeBytes": 200}]
    with pytest.raises(AttachmentTooLargeError):
        svc.create(
            project_id="p-1", target_kind="milestone", target_id="m-1",
            body=None, attachments=atts, caller_user_id="u-1",
        )
...
def test_service_rejects_disallowed_extension(monkeypatch):
    ...
    atts = [{"url": "https://x/y.exe", "filename": "y.exe", "sizeBytes": 10}]
...
def test_service_rejects_empty_body_and_attachments():
    """The service-side guard fires independently of the schema validator.

    Monolith parity: this surfaces with the generic ``validation_error``
    identifier (NOT the specific ``comment_body_or_attachment_required``
    code that the prior implementation used).
    """
    ...
    with pytest.raises(ValidationError):
        svc.create(
            project_id="p-1", target_kind="milestone", target_id="m-1",
            body=None, attachments=None, caller_user_id="u-1",
        )
